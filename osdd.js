  ...
  $.getJSON("https://api.github.com/repos/chuckfitzpatricksf/OSDD_test/issues", function (data) {
    ...
    if (data_index > -1) {
      ...
      for (var i = 0; i < lastTeamMember; i++) {
        ...
        email = team_members[i].email || "NoBody@example.com";  //TODO: use a better approach for missing emails?
        ...
        var team_member;
        if (url) {
          team_member = '<span class="span2"><a href="http://' + url + '" target="_blank"><img src="' + gravatar_url + '" title="' + name + ", " + letters + '"/></a><div>' + name + '</div><div>' + letters + '</div></div></span>'
        } else
        {
          team_member = '<span class="span2"><img src="' + gravatar_url + '" title="' + name + letters + '"/><div>' + name + '</div><div>' + letters + '</div></div></span>'
        }
...
function loadLatestGithubIssues() {
  ...
  $.getJSON("https://api.github.com/repos/OSDDMalaria/OSDDMalaria_To_Do_List/issues", function (data) {
    ...
    for (var i = 0; i < lastGithubItem; i++) {
      ...
      $("#github-feed").append('<span class=' + data[i].state + '><span class=title>' + createdDate + " ---" + item_title + '</span></span>');
...
function get_gravatar(email, size) {
  ...
  if (email) {
    ...
    return 'http://www.gravatar.com/avatar/' + MD5("OSDDMalaria@OSDDMAlaria.com") + '.jpg?s=' + size;
  }
}